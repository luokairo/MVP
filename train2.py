...
def build_everything(args: arg_util.Args):
    ...
    vae_local, var_wo_ddp = build_vae_var(
        ...
        n_cond_embed=args.n_cond_embed, strength=args.strength,
    )
    ...
    names, paras, para_groups = filter_params(var_wo_ddp, nowd_keys={
        ...
        'ada_gss', 'moe_bias',
        'scale_mul',
        'visual_prompt',  # 添加visual prompt到nowd_keys
    })
    
    # 过滤掉不需要训练的参数
    filtered_names = []
    filtered_paras = []
    filtered_para_groups = []
    
    for i, (name, para) in enumerate(zip(names, paras)):
        if para.requires_grad:
            filtered_names.append(name)
            filtered_paras.append(para)
            filtered_para_groups.append(para_groups[i])
    
    print(f"Filtered parameter groups for optimizer: {len(filtered_names)}")
    print(f"Filtered parameter names: {filtered_names}")
    ...
    var_optim = AmpOptimizer(
        ...
        optimizer=opt_clz(params=filtered_para_groups, **opt_kw),
        names=filtered_names, 
        paras=filtered_paras,
        grad_clip=args.tclip, 
        n_gradient_accumulation=args.ac
    )
    del names, paras, para_groups, filtered_names, filtered_paras, filtered_para_groups
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_or_itrt, iters_train: int, trainer):
    ...
    for it, (inp, label, label_name, caption) in me.log_every(start_it, iters_train, ld_or_itrt, 30 if iters_train > 8000 else 5, header):
